 ...
 */
public class ChessStateSerializer extends AbstractStateSerializer<ChessState> {

    @Override
    public String traverseToString(ChessState state) {
        return visitState(state).toString();
    }

    @Override
    public JSONObject traverseToJson(ChessState state) throws NullPointerException {
        return visitState(state);
    }

    private JSONObject visitState(ChessState state) throws NullPointerException {

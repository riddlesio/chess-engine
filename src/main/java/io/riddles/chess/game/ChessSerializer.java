 ...
 */
public class ChessSerializer extends AbstractGameSerializer<ChessProcessor, ChessState> {
    ...
    @Override
    public String traverseToString(ChessProcessor processor, ChessState initialState) {
        ...
        ChessState state = initialState;
        int counter = 0;
        while (state.hasNextState()) {
            state = (ChessState) state.getNextState();
